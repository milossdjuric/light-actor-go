...
func (a *ParentActor) Receive(ctx actor.ActorContext) {
	...
	case string:
		if msg == "SpawnChild" {
			...
			//We use escalate strategy, child fails as well, and parent handles the failure
			childProps := actor.NewActorPropsWithStrategies(self, actor.NewEscalateStrategy(), actor.NewEscalateStrategy()) // Child actor will not have a parent
...
func (a *ChildActor) Receive(ctx actor.ActorContext) {
	...
	case string:
		...
		if msg == "SpawnGrandchild" {
			...
			//We call NotPanic failure, grandchild fails, but child handles the failure
			actorSystem := ctx.ActorSystem()
			actorSystem.EscalateFailureNotPanic("NotPanic", grandChildPID)
